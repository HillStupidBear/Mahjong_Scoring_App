    ...
    <div id="app">
        ...
        <table class="resultTable" v-show="resultShow">
            ...
            <tr>
                ...
                <td><input class="score" type="number" v-model.number="score[0]"/></td>
            </tr>
            <tr>
                <td>二位</td>
                <td>
                ...
                <td><input class="score" type="number" v-model.number="score[1]"/></td>
            </tr>
            <tr>
                <td>三位</td>
                <td>
                ...
                <td><input class="score" type="number" v-model.number="score[2]"/></td>
            </tr>
            <tr>
                <td>四位</td>
                <td>
                ...
                <td><input class="score" type="number" v-model.number="score[3]"/></td>
            </tr>
            <tr>
                <td>
                    <div class="total">
        ...
        methods: {
            ...
            submit: function() {
                if (Number(this.totalScore) !== 120000) {
                    ...
                    alert("请正确选择选手")
                } else if (Number.parseInt(this.score[0]) < this.score[1] || this.score[1] < this.score[2] || this.score[2] < this.score[3]) {
                    alert("请输入正确的顺位")
                } else {
                    for (var player = 0; player < this.items.length; player++) {
                        if (this.items[player].name == this.firPlayer) {
                            this.items[player].score += (Number.parseInt(this.score[0]) - Number.parseInt(30000)) / Number.parseInt(1000) + Number.parseInt(15)
                            this.items[player].topNum += 1
                            this.items[player].playTimes += 1
                        }
                        if (this.items[player].name == this.secPlayer) {
                            this.items[player].score += (Number.parseInt(this.score[1]) - Number.parseInt(30000)) / Number.parseInt(1000) + (5)
                            this.items[player].playTimes += 1
                        }
                        if (this.items[player].name == this.thiPlayer) {
                            this.items[player].score += (Number.parseInt(this.score[2]) - Number.parseInt(30000)) / Number.parseInt(1000) - (5)
                            this.items[player].playTimes += 1
                        }
                        if (this.items[player].name == this.forPlayer) {
                            this.items[player].score += (Number.parseInt(this.score[3]) - Number.parseInt(30000)) / Number.parseInt(1000) - (15)
                            this.items[player].playTimes += 1
                        }
                    }
                    this.items.sort((a, b) => b.score - a.score)
